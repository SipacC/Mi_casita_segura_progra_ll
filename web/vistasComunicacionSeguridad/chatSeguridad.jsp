...
<head>
    <meta charset="UTF-8">
    <title>Comunicación Interna - Seguridad</title>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .col-md-9 { width: 100% !important; padding: 0 8px; }
            #mensajes {
        ...
        @media (max-width: 480px) {
            #mensajes { height: calc(100vh - 110px) !important; padding: 10px; }
        }
    </style>
    ...
    <script>
        ...
        function conectarWebSocket() {
            ...
            socket.onopen = () => console.log("✅ WS Seguridad conectado");
        ...
        function seleccionarConversacion(id) {
            ...
            window.location.href = "ControladorComunicacionSeguridad?accion=verchat&idConversacion=" + id;
        }
    ...
    <div class="row">
        ...
        <div class="col-md-3 border-end">
            <div class="d-flex justify-content-between align-items-center">
                <h5>Chats</h5>
                <button class="btn btn-outline-primary btn-sm" onclick="window.location.href='ControladorSeguridad?accion=menu'">🏠 Menú</button>
            </div>
                    ...
                    for (Conversacion c : conversaciones) { %>
                        <li>
                            <button id="chat-<%= c.getIdConversacion() %>" class="btn btn-outline-secondary w-100 text-start mb-1"
                                ...
                                🏠 <%= c.getNombreResidente() != null ? c.getNombreResidente() : "Residente" %>
                            </button>
                        </li>
                <% } } else { %>
        ...
        <div class="col-md-9">
            <% if (request.getAttribute("idConversacion") != null) { %>
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <button class="btn btn-outline-secondary btn-sm btn-volver" onclick="window.location.href='ControladorComunicacionSeguridad?accion=menuchats'">← Volver</button>
                    <h6 class="m-0">Conversación actual</h6>
                </div>

                <div id="mensajes" class="mb-2">
...
<dialog id="modalNuevaConversacion">
    <form method="get" action="ControladorComunicacionSeguridad">
        <input type="hidden" name="accion" value="crearConversacion">
        <h5>Iniciar nueva conversación</h5>
        <p>Selecciona un residente:</p>
        <select name="idResidente" class="form-select" required>
            <option value="">-- Selecciona --</option>
            <%
                ConexionDB db = new ConexionDB();
                UsuarioDAO usuarioDAO = new UsuarioDAO(db.openConnection());
                List<Usuario> residentes = usuarioDAO.listarResidentesActivos();
            ...
            <% } } else { %>
                <option disabled>No hay residentes activos</option>
            <% } db.closeConnection(); %>
        </select>
